	...
	<div>
		...
		<div class="first_item_1">
			...
			<el-form :model="addForm" :rules="addFormRule" ref="addFormRef" label-width="100px" class="demo-dynamic" style="width:70%;margin-left:15%;margin-top:40px">
				...
				<el-form-item label="区域" :required="true">
					<MjbArea :isShowArea="true" :_province.sync="addForm.province" :_city.sync="addForm.city" :_area.sync="addForm.area" />
				</el-form-item>
				...
				<el-form-item label="物业类型" prop="propertyType">
					<el-radio-group size="small" v-model="addForm.propertyTypeRadio" @change="propertyTypeRadioChange">
					...
					<el-input size="small" style="width:200px;margin-left:10px" placeholder="请填写其它选项" v-show="addForm.propertyTypeRadio == '4'" v-model="addForm.propertyType">
					</el-input>
				</el-form-item>

				<el-form-item label="销售状态"  prop="sellStatus">
					<el-radio-group size="small" v-model="addForm.sellStatusRadio" @change="sellStatusRadioChange">
						<el-radio label="0">在售</el-radio>
						<el-radio label="1">待售</el-radio>
						<el-radio label="2">售罄</el-radio>
						<el-radio label="4">其它</el-radio>
					</el-radio-group>
					<el-input size="small" style="width:200px;margin-left:10px" placeholder="请填写其它选项" v-show="addForm.sellStatusRadio == '4'" v-model="addForm.sellStatus">
					</el-input>
				</el-form-item>

				<el-form-item label="开盘日期" prop="sellTime">
					<el-date-picker size="small" format="yyyy-MM-dd" v-model="addForm.sellTime" type="date" placeholder="开盘日期">
					</el-date-picker>
					<span style="color:#999;font-size:12px">主要是以开盘说明为准，这里用于开盘日历及列表排序</span>
				</el-form-item>

				<el-form-item label="交房日期" prop="deliverTime">
					<el-date-picker size="small" format="yyyy-MM-dd" v-model="addForm.deliverTime" type="date" placeholder="交房日期">
					</el-date-picker>
				</el-form-item>
				...
				<el-form-item label="装修程度" prop="decorationdLevel">
					<el-radio-group size="small" v-model="addForm.decorationdLevelRadio" @change="decorationdLevelRadioChange">
						...
						<el-input style="width:200px;margin-left:10px" placeholder="请填写其它选项" v-show="addForm.decorationdLevelRadio == '5'" v-model="addForm.decorationdLevel">
						</el-input>
					</el-radio-group>
				</el-form-item>
				...
				<el-form-item label="建筑类型" prop="buildingType">
					...
					<el-radio-group size="small" v-model="addForm.buildingTypeRadio" @change="buildingTypeRadioChange">
						...
						<el-radio label="5">其它</el-radio>
						<el-input style="width:200px;margin-left:10px" placeholder="请填写其它选项" v-show="addForm.buildingTypeRadio == '5'" v-model="addForm.buildingType">
						</el-input>
					</el-radio-group>
				</el-form-item>

				<el-form-item label="现房期房" prop="housingType">
				...
				<el-form-item label="楼盘标签" prop="housingLable">
					<el-checkbox-group size="small" v-model="addForm.housingLable">
						<el-checkbox :label="item.lableId" v-for="(item,index) in houseLableList" :key="index">
							{{item.lableName}}
						</el-checkbox>
					</el-checkbox-group>
				</el-form-item>
		...
		<div class="first_item_3">
			...
			<el-form :model="addForm" label-width="100px" class="demo-dynamic" style="width:60%;margin-left:15%;margin-top:40px">
				...
				<!-- <div v-for="(item,index) in addForm.permitPresaleList" :key="index" style="border-top:1px solid #ccc;padding-top:10px">
					...
					<el-form-item label="发证时间" style="width:600px">
						<el-date-picker size="small" format="yyyy-MM-dd" v-model="item.createTime" type="date" onPick="pick" placeholder="发证时间">
						</el-date-picker>
					</el-form-item>
                ...
                <el-form-item  label="楼盘亮点" style="width:600px;;padding:10px 0;" prop="brightList2" >
                    <el-checkbox-group v-model="addForm.brightList2">
                        <el-checkbox :label="item.id" 
                                                :disabled="item.id == 6 || item.id == 3? false:true" 
                                                v-for="(item,index) in addForm.brightList" 
                                                :key="index">{{item.brightName}}</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
			</el-form>
		</div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      addForm: {
        ...
        brightList:[
        {brightName: "低密度", brightStatus: 0, id: 1},
        {brightName: "品牌房企", brightStatus: 0, id: 2},
        {brightName: "车位充足", brightStatus: 0, id: 3},
        {brightName: "高绿化率", brightStatus: 0, id: 4},
        {brightName: "品质物业", brightStatus:0, id: 5},
        {brightName: "鹰眼精品", brightStatus: 0, id: 6}
        ],
        brightList2:[],
      },
      //添加新的楼盘基本信息表单验证规则
      addFormRule: {
        buildingName: [{ validator: validateTitle, trigger: "blur" }],
        // propertyType: [{ validator: validateType, trigger: "blur" }],
        // sellStatus: [{ validator: validateState, trigger: "blur" }],
        // decorationdLevel: [
        //   { validator: validateDecorationDegree, trigger: "blur" }
        // ],
        // storey: [{ validator: validateFloor, trigger: "blur" }]
        // housingType: [{ validator: validateExisting, trigger: "blur" }]
      },
      //地图标记以及中心位置
      markers: [],
  ...
  methods: {
      ...
      this.$http("/backstageBuilding/selBuildingInfo", {}, { body }, {}).then(
        function(res) {
          if (res.data.code == 0) {
            ...
            // _this.addForm =_.cloneDeep(res.data.response);
            if (
            ...
            if (
              ...
              _this.addForm.decorationdLevelRadio =
                res.data.response.decorationdLevel; 
            } else {
            ...
            _this.addForm.brightList.forEach((item,index) => {
                
                if(item.brightStatus == 1){
                    _this.addForm.brightList2.push(item.id);
                }
            })
            // _this.addForm.brightList2 = _this.addForm.brightList2.join(",").split(",");

            _this.addForm.mapLon =
              _this.addForm.mapLon === "" ? 0 : _this.addForm.mapLon;
            _this.addForm.mapLat =
              _this.addForm.mapLat === "" ? 0 : _this.addForm.mapLat;
    ...
    add_new_permitpresale() {
      this.addForm.permitPresaleList.push({
        permitName: "",
        createTime: "",
        building: ""
      });
        ...
        if (valid) {
          _this
            ...
            .then(() => {
              ...
              console.log(body.brightList2);
              let ar1=[],ar2 = [];
             body.brightList2.forEach((item,index) => {
                if(item == 3){
                    ar1.push(item)
                }else if(item == 6){
                    ar2.push(item)
                ...
                if(ar1.length >0){
                     body.brightList[2].brightStatus= 1;
                 }else{
                     body.brightList[2].brightStatus= 0;
                 } 
                 if(ar2.length > 0){
                     body.brightList[5].brightStatus= 1;
                 }else{
                     body.brightList[5].brightStatus= 0;
                 }
