				...
				<el-col :span="20">
					<el-form :inline="true" style="" :model="filterForm" class="demo-form-inline">
					  ...
					  <el-form-item label="单元">
						...
						placeholder="不限">
						  <el-option v-for="(item,index) in bUnitArray" :key="item.id" :label="item.id" :value="item.id"></el-option>
						</el-select>
					  </el-form-item>
			...
			<BigDialog dialogTitle="添加报告" :dialogFormVisible="dialogFormVisible" @dialogCancel="dialogCancel" @dialogConfirm="dialogConfirm(1)">
				<div slot="dia_body">
					<el-form style="" :model="paParkDetail" class="demo-form-inline" label-width="150px">
						...
						v-model="paParkDetail.bBan" 
						placeholder="楼栋号">
						  <el-option v-for="(item,index) in bBanArray2" :key="item.id" :label="item.id" :value="item.id"></el-option>
						</el-select>

						<el-select 
						size="small" 
						clearable 
						:disabled="paParkDetail.bBan==''" 
						@change="conditionChange('bUnit',2)" 
						v-model="paParkDetail.bUnit" 
						placeholder="单元">
						  <el-option v-for="(item,index) in bUnitArray2" :key="item.id" :label="item.id" :value="item.id"></el-option>
						</el-select>
					  
						<el-select 
						size="small" 
						clearable 
						:disabled="paParkDetail.bBan=='' || paParkDetail.bUnit==''" 
						@change="conditionChange('bDiv',2)" 
						v-model="paParkDetail.bDiv" placeholder="楼层">
						  <el-option v-for="(item,index) in bDivArray2" :key="item.id" :label="item.id" :value="item.id"></el-option>
						</el-select>
					  
						<el-select 
						size="small" 
						clearable 
						:disabled="paParkDetail.bBan=='' || paParkDetail.bUnit=='' || paParkDetail.bDiv==''" 
						v-model="paParkDetail.bNum" 
						placeholder="户号">
						  <el-option v-for="(item,index) in bNumArray2" :key="item.id" :label="item.id" :value="item.id"></el-option>
						</el-select>
				...
				<div slot="dia_body">
					<el-form style="" :model="paParkDetail" class="demo-form-inline" label-width="150px">
			            ...
			            <el-form-item label="位置：">
			            	...
			            	<span>单元{{paParkDetailView.bUnit}}</span>
			            	<span>楼层{{paParkDetailView.bDiv}}</span>
			            	<span>户号{{paParkDetailView.bNum}}</span>
			            </el-form-item>
	...
	export default{
		...
		methods:{
			...
			initNav(){
				...
				path2 = '/index/estateindoorreportList?type=edit&buildingId='+q.buildingId+'&buildingName='+q.buildingName+'&activeName='+q.activeName;
	      		this.navList = [
					{path:'/index/estateprocessmonitoringmanagement',name:'首页'},
					{path:path1,name:buildingName},
					{path:path2,name:'户内报告'},
			...
			conditionChange(name,type){
				if(name === 'bBan'){
					if(type ==1 ){
						...
						this.paParkDetail.bUnit = '';
						this.paParkDetail.bDiv = '';
						this.paParkDetail.bNum = '';
					}
				}else if(name === 'bUnit'){
					if(type ==1 ){
						this.filterForm.bDiv = '';
				...
				if(type == 1){
					...
					body = {
						buildingId:this.$route.query.buildingId,
						buildingNum:this.paParkDetail.bBan,
						unitNum:this.paParkDetail.bUnit,
						floor:this.paParkDetail.bDiv,
						accountNum:this.paParkDetail.bNum
					};
				};
				if(body.buildingId == ''){
			...
			dialogConfirm(type){
				if(type == 1){
					...
					if(this.paParkDetail.bBan == ''){
						message(this,'请选择楼栋号','warning');
						return;
					}
					if(this.paParkDetail.bUnit == ''){
						message(this,'请选择单元','warning');
						return;
					}
					if(this.paParkDetail.bDiv == ''){
						message(this,'请选择楼层','warning');
						return;
					}
					if(this.paParkDetail.bNum == ''){
						message(this,'请选择户号','warning');
						return;
					}
			          ...
			          if(res.data.code == 0){
			          	if(res.data.response.status == 1){
			          		...
			          		if(_this.handleType == 'add'){
								...
								_this.filterForm.bUnit = '';
								_this.filterForm.bDiv = '';
								_this.filterForm.bNum = '';
				...
				if(type == 1){
					...
					for(var i in row){
						if(i != 'bBan' && i != 'bUnit' && i != 'bDiv' && i != 'bNum'){
							this.paParkDetail[i] = row[i];
						}
					}

					this.$nextTick(function(){
						this.paParkDetail.bBan = row.bBan;
						this.$nextTick(function(){
							this.paParkDetail.bUnit = row.bUnit;
							this.$nextTick(function(){
								this.paParkDetail.bDiv = row.bDiv;
								this.$nextTick(function(){
									this.paParkDetail.bNum = row.bNum;
								})
							})
						})
					})
