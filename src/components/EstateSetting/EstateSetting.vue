  ...
  <div>
    <el-form :model="form" label-width="150px" class="demo-dynamic">
      ...
      <el-form-item label="楼盘群聊">
        <el-radio-group v-model="form.configList[5].configValue">
          <el-radio :label="1">启动</el-radio>
          <el-radio :label="0">停止</el-radio>
        </el-radio-group>
        <el-button type="text" class="submitBtn" @click="submit(5)">保存</el-button>
      </el-form-item>
      <el-form-item label="开盘变价订阅通知">
        <el-radio-group v-model="form.configList[6].configValue">
          <el-radio :label="1">启动</el-radio>
          <el-radio :label="0">停止</el-radio>
        </el-radio-group>
        <el-button type="text" class="submitBtn" @click="submit(6)">保存</el-button>
      </el-form-item>
      <!-- <el-form-item label="户型介绍">
        <el-radio-group v-model="form.configList[7].configValue">
          <el-radio :label="1">启动</el-radio>
          <el-radio :label="0">停止</el-radio>
        </el-radio-group>
        <el-button type="text" class="submitBtn" @click="submit(7)">保存</el-button>
      </el-form-item> -->
    </el-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getSettingData() {
      ...
      this.$http("/backstageBuilding/getBuildingConfig", {}, { body }, {})
        .then(function(res) {
          if (res.data.code == 0) {
    ...
    submit(configType, index) {
      ...
      body.configValue = this.form.configList[configType].configValue;
