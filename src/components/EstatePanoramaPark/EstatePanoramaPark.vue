<template>
  <div>
    <el-row style="border:1px solid #ccc;padding:20px 10px 0px">
      ...
      <el-col :span="4">
        <el-button style="float:right;margin-left:5px;" type="primary" @click="addNew" v-if="type==='edit'">添加园区全景
    ...
    <BigDialog dialogTitle="园区全景详情" :isShowCancel="false" :dialogFormVisible="viewDialogFormVisible"
               @dialogCancel="dialogCancel(2)" @dialogConfirm="dialogConfirm(2)">
      <div slot="dia_body">
        <el-form style="" class="demo-form-inline" label-width="150px">
          <el-form-item label="园区全景图名称：">
            <span>{{paParkDetail.pName}}</span>
          </el-form-item>
          <el-form-item label="园区全景图地址：">
            <span :href="paParkDetail.httpUrl" style="word-wrap:break-word" target="_blank">{{paParkDetail.httpUrl}}</span>
            <el-button type="text" @click="addSee" style="margin-left:16px;">查看</el-button>
          </el-form-item>
  ...
  export default{
    ...
    addSee(){
        this.geyairscapeUrl2();
         window.open(this.paParkDetail.httpUrl+ '&d='+this.addMassage);
    },
     geyairscapeUrl2(){
              let [_this,body] = [this,{}]
